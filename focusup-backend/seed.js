...
// Prevent accidental seeding in production unless explicitly forced
if (process.env.NODE_ENV === 'production' && !process.argv.includes('--force')) {
  console.log('⚠️ Seeding is disabled in production. Rerun with --force to override.');
  process.exit(1);
}
...
const seedResources = async (users, groups) => {
  const createdContents = [];

  for (const group of groups) {
    // Pick the group's creator as the content owner when possible
    const owner = users.find(u => String(u._id) === String(group.createdBy)) || users[0];

    const pdf = {
      userId: owner._id,
      title: `${group.name} - Intro PDF`,
      type: 'pdf',
    ...
    const note = {
      userId: owner._id,
      title: `${group.name} - Starter Notes`,
      type: 'note',
      description: `Starter notes for ${group.name}`,
      isPublic: true,
    };

    const created = await Content.insertMany([pdf, youtube, note]);

    // Attach the created content as group resources
    const resourcesToAdd = created.map(c => ({
      id: String(c._id),
      title: c.title,
      link: c.url || '',
      type: c.type,
    ...
    await Group.updateOne({ _id: group._id }, { $push: { resources: { $each: resourcesToAdd } } });

    createdContents.push(...created);
  }

  console.log(`✅ Created ${createdContents.length} content resources and attached them to groups`);
...
const seedDatabase = async () => {
  try {
    ...
    const resources = await seedResources(users, groups);
