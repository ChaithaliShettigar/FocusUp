...
export const NavBar = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-30 bg-[rgba(247,242,233,0.9)] backdrop-blur-md border-b border-clay/60">
      <div className="mx-auto flex w-full max-w-[1600px] items-center justify-between px-6 md:px-12 lg:px-20 py-3">
              ...
              <div className="relative">
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2 w-40 bg-white shadow-lg rounded-md py-1 z-20">
                    {isAuthenticated ? (
                      <>
                        <div className="px-4 py-2 border-b border-clay/60 mb-1">
                          <p className="text-sm font-semibold text-ink">{user?.name || 'User'}</p>
                          <p className="text-xs text-ink/70 truncate">{user?.email || ''}</p>
                        </div>
                        <NavLink
                          to="/profile"
